...
if submit_button:
    ...
    if download_button:
        with st.spinner('Processing...'):
            ...
            output_dir = os.path.join(home_dir, "Downloads", "py")
            os.makedirs(output_dir, exist_ok=True)
            filename = url.split('/')[-1].split('.')[0]
            output_csv_file = os.path.join(output_dir, f"{filename}.csv")

            # Save the output dataframe to the output file
            output_df.to_csv(output_csv_file, index=False)

            # Provide a download link for the output file
            st.markdown(f"[Download {os.path.basename(output_csv_file)}](data:file/csv;base64,{output_df.to_csv(index=False).encode('utf-8').decode()})", unsafe_allow_html=True)
